...
import { moveRigidBody, applyForce, moveBall } from "./player";
...
export function handleKeys(delta, g_currentlyPressedKeys) {
	if (g_currentlyPressedKeys['KeyU']) {	//H
		...
		applyImpulse(cube.userData.physicsBody, 5, {x:0, y:1, z:0});
	}
    const movableMesh = g_scene.getObjectByName("box");
	if (g_currentlyPressedKeys['KeyA']) {	//A
		moveBall({left: 1, right: 0, forward: 0, back: 0})
	}
	if (g_currentlyPressedKeys['KeyD']) {	//D
        moveDirection.right = 1;
		moveBall({left: 0, right: 1, forward: 0, back: 0})
	}
	if (g_currentlyPressedKeys['KeyW']) {	//W
        moveDirection.forward = 1;
		moveBall({left: 0, right: 0, forward: 1, back: 0})
	}
	if (g_currentlyPressedKeys['KeyS']) {	//S
        moveDirection.back = 1;
		moveBall({left: 0, right: 0, forward: 0, back: 1})
	}
}
