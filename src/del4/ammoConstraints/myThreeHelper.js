...
import {pushHingedArm} from "./hingedarm.js";
...
export function handleKeys(delta, g_currentlyPressedKeys) {
	...
	const armMesh = g_scene.getObjectByName("hinge_arm");
	...
	if (g_currentlyPressedKeys['KeyV']) {
		pushHingedArm(armMesh, direction);
	}
	if (g_currentlyPressedKeys['KeyB']) {
		pushHingedArm(armMesh, oppositeDirection);
	}

	const movableMesh = g_scene.getObjectByName("movable");
